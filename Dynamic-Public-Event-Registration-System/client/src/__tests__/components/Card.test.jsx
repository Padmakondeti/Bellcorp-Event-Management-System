...
describe('Card Component', () => {
  it('renders all event details correctly', () => {
    ...
    // Image alt text
    expect(
      screen.getByAltText(/Banner for React Conference 2026/i),
    ).toBeInTheDocument()

    // Title
    expect(
      screen.getByRole('heading', {name: /React Conference 2026/i}),
    ).toBeInTheDocument()

    // Location and time
    expect(screen.getByText(/📍 Hyderabad | ⏰ 10:00 AM/i)).toBeInTheDocument()

    // Description
    expect(
      screen.getByText(/Join us for a deep dive into React/i),
    ).toBeInTheDocument()
    ...
    // Date
    expect(screen.getByText(/📅 2026-02-14/i)).toBeInTheDocument()

    // Button
    expect(
      screen.getByRole('button', {
        name: /View details for React Conference 2026/i,
      }),
    ).toBeInTheDocument()
  })
  ...
  it('has accessible roles and labels', () => {
    ...
    expect(button).toHaveAccessibleName(
      'View details for React Conference 2026',
    )
  })
