...
describe('EventDetails', () => {
  ...
  it('shows loading state initially', async () => {
    eventService.getEventById.mockReturnValue(new Promise(() => {})) 
  ...
  it('renders full event details when data is available', async () => {
    ...
    expect(screen.getByText(/date/i)).toHaveTextContent('📅 Date: 2026-02-01')
    ...
    expect(screen.getByText(/seats availability/i)).toHaveTextContent(
      'Seats Availability:',
    )
  ...
  it('renders navigation buttons', async () => {
    ...
    expect(
      screen.getByRole('link', {name: /register for react workshop/i}),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('link', {name: /browse more events/i}),
    ).toBeInTheDocument()
  })
})
