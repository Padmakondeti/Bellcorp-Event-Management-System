...
describe('EventRegistrants', () => {
  ...
  it('filters registrants by search term', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Padmavathi')).not.toBeInTheDocument()
      expect(screen.getByText('Bob')).toBeInTheDocument()
    })
  })
  ...
  it('removes a single registrant', async () => {
    ...
    await screen.findByText('Padmavathi')
    fireEvent.click(screen.getByLabelText(/remove registrant padmavathi/i))
  ...
  it('calls exportToCSV with correct data', async () => {
    ...
    expect(csvService.exportToCSV).toHaveBeenCalledWith(
      ...
      [
        ['Padmavathi', 'padmavathi@example.com', '1234567890', 'Excited to join!'],
  ...
  it('shows error message if registrants fetch fails', async () => {
    registrationService.getRegistrations.mockRejectedValueOnce(
      new Error('fail'),
    )
