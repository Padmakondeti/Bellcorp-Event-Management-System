...
describe('AdminDashboard', () => {
  test('renders admin greeting and stats', async () => {
    ...
    expect(await screen.findByText(/Welcome, Padma/i)).toBeInTheDocument()
  ...
  test('renders event table with correct data', async () => {
    ...
    const table = await screen.findByRole('table', {
      name: /List of all events/i,
    })
    expect(getAllByTextContent(table, 'React Summit')).toHaveLength(5)
    expect(getAllByTextContent(table, '2026-02-01')).toHaveLength(4)
  ...
  test('renders all action buttons', () => {
    ...
    expect(
      screen.getByRole('button', {name: /View contact messages/i}),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {name: /View all registrants/i}),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {name: /Add new event/i}),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {name: /Register new admin/i}),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {name: /Manage user feedback/i}),
    ).toBeInTheDocument()
  ...
  test('deletes an event with confirmation and loading state', async () => {
    ...
    const deleteButton = await screen.findByRole('button', {
      name: /Delete event titled React Summit/i,
    })
  ...
  test('exports registrants for an event', async () => {
    ...
    const exportButton = await screen.findByRole('button', {
      name: /Export registrants for event titled React Summit/i,
    })
    fireEvent.click(exportButton)
    await waitFor(() =>
      expect(
        screen.getByText(/Registrants exported successfully/i),
      ).toBeInTheDocument(),
    )
  })
})
