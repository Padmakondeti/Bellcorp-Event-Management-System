...
function Home() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success(
        '✅Thanks for your Feedback submitted ☺️! Awaiting admin approval.',
      )
  ...
  const allTags = [
    ...new Set(
      featuredEvents.flatMap(
        ev => ev.tags?.split(',').map(tag => tag.trim()) || [],
      ),
    ),
  ]

  const filteredEvents = featuredEvents.filter(event => {
    const matchesSearch = `${event.title} ${event.location}`
      .toLowerCase()
    ...
    <div className='min-h-screen w-full bg-gradient-to-br from-indigo-50 to-white dark:from-gray-900 dark:to-gray-800 text-[#1F2937] dark:text-gray-200'>
      ...
      <div className='relative w-full h-screen max-md:h-fit overflow-hidden'>
        <video
          ...
          muted
          loop
          className='absolute top-0 left-0 w-full h-full object-cover z-0 brightness-[0.6] dark:brightness-[0.4]'
        >
        ...
        <section className='relative z-10 px-6 py-24 text-center flex flex-col items-center justify-center h-full'>
          ...
          <p className='text-lg max-md:text-base text-gray-700 dark:text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed'>
            Discover upcoming events, register with ease, and stay informed.
      ...
      <section className='w-full px-4 sm:px-6 lg:px-8 py-20'>
        ...
        {filteredEvents.length === 0 ? (
          <p className='text-gray-600 dark:text-gray-400 text-center'>
            No events match your search or selected tag.
          </p>
        ) : (
      ...
      <section className='px-6 py-16 text-center bg-[#F3F4F6] dark:bg-gray-900'>
        <blockquote className='italic text-lg text-gray-700 dark:text-gray-300 max-w-xl mx-auto leading-relaxed'>
          “This platform made it so easy to handle Registrations, updates, and communication — all in one place!”
        </blockquote>
        <p className='mt-4 text-sm text-gray-500 dark:text-gray-400 font-medium'>
          — Event Organizer, Bellcorp
        </p>
      </section>
      ...
      <section className='bg-[#DAC9F1] dark:bg-indigo-900 py-16 px-6 text-center text-[#1F2937] dark:text-white'>
        ...
        <div className='flex flex-wrap justify-center gap-6'>
          {[
            ...
            <div
              key={idx}
              className='bg-[#F3F4F6] dark:bg-gray-800 w-full md:w-1/4 p-6 rounded-2xl shadow hover:scale-105 transition text-center'
            >
      ...
      <section className='bg-[#F3F4F6] dark:bg-gray-900 py-16 px-6'>
        <h1 className='text-4xl font-bold text-center mb-10 text-[#1F2937] dark:text-yellow-300'>
        ...
        <div className='space-y-8 max-w-4xl mx-auto'>
          {[
            ...
            <div key={idx}>
              <h2 className='text-2xl font-bold text-[#1F2937] dark:text-yellow-300'>
                {faq.q}
              </h2>
      ...
      <section className='bg-[#DAC9F1] dark:bg-indigo-900 py-16 px-6 text-center text-[#1F2937] dark:text-white'>
        ...
        <div className='flex flex-wrap justify-center gap-6'>
          {[
            ...
            <div
              key={idx}
              className='bg-white dark:bg-gray-800 w-full md:w-[22%] p-6 rounded-xl shadow hover:scale-105 transition text-center'
            >
              <div className='flex justify-center items-center text-5xl text-indigo-600 dark:text-yellow-300 mb-4'>
                {feature.icon}
              </div>
      ...
      <section className='bg-white dark:bg-gray-900 py-16 px-6 text-center text-[#1F2937] dark:text-white'>
        ...
        <div className='flex flex-wrap justify-center gap-6'>
          {[
            ...
            <div
              key={idx}
              className='bg-[#F3F4F6] dark:bg-gray-800 w-full md:w-[28%] p-6 rounded-xl shadow text-center hover:scale-105 transition'
            >
              <div className='text-4xl font-bold text-blue-600 dark:text-yellow-300'>
                {item.step}
              </div>
        ...
        >
          ...
          <div className='space-y-2'>
            ...
            {wordCount > MAX_WORDS && (
              <p className='text-red-500 text-xs'>
                Comment exceeds the 30-word limit.
              </p>
            )}
          </div>
          ...
          <button
            ...
            className={`w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 dark:from-yellow-400 dark:to-yellow-500 text-white dark:text-gray-900 font-bold py-3 px-6 rounded-lg transition focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:focus-visible:ring-yellow-400 shadow-md ${
              wordCount > MAX_WORDS ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
            🚀 Submit Feedback
          </button>
      ...
      <section className='bg-[#DAC9F1] dark:bg-indigo-900 py-16 px-6 text-center text-[#1F2937] dark:text-white'>
        ...
        <p className='text-lg mb-8'>
          Don’t wait — seats are limited and events are happening now. Be part
          of the experience.
        </p>
        <button
          onClick={() => navigate('/events')}
