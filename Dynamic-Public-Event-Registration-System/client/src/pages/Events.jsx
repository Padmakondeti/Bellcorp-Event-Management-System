...
function Events() {
  ...
  const allTags = [
    ...new Set(
      events.flatMap(ev => ev.tags?.split(',').map(tag => tag.trim()) || []),
    ),
  ]
      ...
      <section className='mb-16'>
        <div className='max-w-6xl mx-auto px-4'>
          <header className='relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-3xl shadow-xl px-6 py-12 md:px-12 md:py-16 text-center border border-indigo-100 dark:border-gray-700 overflow-hidden'>
            ...
            <div
              aria-hidden='true'
              className='absolute top-[-80px] left-[10%] w-72 h-72 bg-gradient-to-br from-indigo-400 to-purple-500 dark:from-yellow-400 dark:to-yellow-600 opacity-30 rounded-full blur-3xl pointer-events-none'
            ></div>
            <div
              aria-hidden='true'
              className='absolute bottom-[-100px] right-[5%] w-64 h-64 bg-gradient-to-tr from-pink-300 to-indigo-400 dark:from-yellow-600 dark:to-yellow-400 opacity-20 rounded-full blur-3xl pointer-events-none'
            ></div>
            ...
            <p className='relative z-10 mt-6 text-gray-700 dark:text-gray-300 text-lg md:text-xl leading-relaxed max-w-3xl mx-auto'>
              Browse a wide range of exciting events including hands-on
              workshops, cutting-edge tech talks, vibrant cultural festivals,
              and more — all curated to match your interests and happening right
              around you.
            </p>

            {/* Filters */}
            <form
              className='relative z-10 mt-10 flex flex-col md:flex-row gap-4 justify-center items-center'
      ...
      <section aria-label='Event results'>
        {filteredEvents.length > 0 ? (
          ...
          <p
            role='status'
            aria-live='polite'
            className='text-center mt-32 text-sm text-gray-500 dark:text-gray-400'
          >
