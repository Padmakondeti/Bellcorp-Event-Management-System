...
describe('Admin Routes', () => {
  ...
  test('registers a new admin successfully', async () => {
    ...
    const adminInDb = await Admin.findOne({username: 'padmavathi'})
    expect(adminInDb).not.toBeNull()
    expect(adminInDb.email).toBe('padma@example.com')
  })
  ...
  test('logs in with correct credentials', async () => {
    ...
    expect(res.body.name).toBe('Padmavathi Kondeti')
  })
